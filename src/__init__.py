...
def build_plot(
    ...
    X_test, y_test,
    left=0.0, right=0.5,
    step=0.001, l_rate=0.001, it=100
):
    '''Строит график зависимости MSE (на тестовой выборке) от коэф. L1-регрессии'''
    xs = np.arange(left, right + step, step)
    errors = []
    for x in xs:
        regr = GradientLR(l_rate, iterations=it, coef=x)
...
if __name__ == __main__:
    ...
    build_plot(X_train, y_train, X_test, y_test, right=2, step=0.025, l_rate=0.001)

    # Результаты применения обычной линейной регрессии
    print("\nNormalLR:")
    names = ['Уровень преступности на душу населения',
             'Доля жилой земли, выделенной для участков пл. более 25 т. кв. футов',
             ...
             'Взвешенное расстояние до 5 глав. бостонских центров трудоустройства',
             ...
             'Вел. 1000*(Bk - 0.63)^2, где Bk- доля чернокож. населения на район',
